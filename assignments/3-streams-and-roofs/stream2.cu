...
#   define STREAM_ARRAY_SIZE	21000000
#endif
...
/* But we are going to transfer from host to device each time */
...
extern int    checkSTREAMresults(dim3 grid, dim3 block);
...
main()
{
  ...
  err = cudaHostAlloc(&h_a, STREAM_ARRAY_SIZE * sizeof(STREAM_TYPE), cudaHostAllocDefault); CUDA_CHK(err);
  err = cudaHostAlloc(&h_b, STREAM_ARRAY_SIZE * sizeof(STREAM_TYPE), cudaHostAllocDefault); CUDA_CHK(err);
  err = cudaHostAlloc(&h_c, STREAM_ARRAY_SIZE * sizeof(STREAM_TYPE), cudaHostAllocDefault); CUDA_CHK(err);

  /* Get initial value for system clock. */
  STREAM_Init_1D<<<grid, block>>>();
  err = cudaMemcpyFromSymbolAsync(h_a, a, STREAM_ARRAY_SIZE * sizeof(STREAM_TYPE), 0, cudaMemcpyDeviceToHost); CUDA_CHK(err);
  err = cudaMemcpyFromSymbolAsync(h_b, b, STREAM_ARRAY_SIZE * sizeof(STREAM_TYPE), 0, cudaMemcpyDeviceToHost); CUDA_CHK(err);
  err = cudaMemcpyFromSymbolAsync(h_c, c, STREAM_ARRAY_SIZE * sizeof(STREAM_TYPE), 0, cudaMemcpyDeviceToHost); CUDA_CHK(err);
  ...
  t = mysecond();
  err = cudaMemcpyToSymbolAsync(a, h_a, STREAM_ARRAY_SIZE * sizeof(STREAM_TYPE), 0, cudaMemcpyHostToDevice); CUDA_CHK(err);
  ...
  for (k=0; k<NTIMES; k++)
  {
    times[0][k] = mysecond();
    err = cudaMemcpyToSymbolAsync(a, h_a, STREAM_ARRAY_SIZE * sizeof(STREAM_TYPE), 0, cudaMemcpyHostToDevice); CUDA_CHK(err);
    err = cudaMemcpyToSymbolAsync(c, h_c, STREAM_ARRAY_SIZE * sizeof(STREAM_TYPE), 0, cudaMemcpyHostToDevice); CUDA_CHK(err);
    ...
    times[1][k] = mysecond();
    err = cudaMemcpyToSymbolAsync(b, h_b, STREAM_ARRAY_SIZE * sizeof(STREAM_TYPE), 0, cudaMemcpyHostToDevice); CUDA_CHK(err);
    err = cudaMemcpyToSymbolAsync(c, h_c, STREAM_ARRAY_SIZE * sizeof(STREAM_TYPE), 0, cudaMemcpyHostToDevice); CUDA_CHK(err);
  ...
  err = cudaFreeHost(h_a); CUDA_CHK(err);
  err = cudaFreeHost(h_b); CUDA_CHK(err);
  err = cudaFreeHost(h_c); CUDA_CHK(err);
...
#endif
int checkSTREAMresults (dim3 grid, dim3 block)
{
  ...
  h_sum[1] = 0.0;
  h_sum[2] = 0.0;
  err = cudaMemcpyToSymbolAsync(d_sum, h_sum, 3 * sizeof(double), 0, cudaMemcpyHostToDevice); CUDA_CHK(err);
  STREAM_Sum<<<grid, block, block.x * sizeof(double)>>>();
  err = cudaMemcpyFromSymbolAsync(h_sum, d_sum, 3 * sizeof(double), 0, cudaMemcpyDeviceToHost); CUDA_CHK(err);
...
/* vim: set autoindent cindent expandtab ts=2 sw=2: */
