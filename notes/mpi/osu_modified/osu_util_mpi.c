...
void set_buffer_pt2pt_sr (void * buffer, int rank, enum accel_type type, int data, size_t size, int sendid, int recvid)
{
    char buf_type = 'H';
    ...
    if (options.bench == MBW_MR) {
        ...
        if (rank != sendid && rank != recvid) return;
        buf_type = (rank == sendid) ? options.src : options.dst;
    }
...
{
    set_buffer_pt2pt_sr(buffer, rank, type, data, size, 0, 1);
}
...
int allocate_memory_pt2pt_sr (char ** sbuf, char ** rbuf, int rank, int sendid, int recvid)
{
    unsigned long align_size = sysconf(_SC_PAGESIZE);
...
{
    return allocate_memory_pt2pt_sr(sbuf, rbuf, rank, 0, 1);
}
...
void free_memory_sr (void * sbuf, void * rbuf, int rank, int sendid, int recvid)
{
    if (rank == sendid) {
...
{
    free_memory_sr(sbuf, rbuf, rank, 0, 1);
}
